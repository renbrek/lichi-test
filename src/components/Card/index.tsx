...
export function Card(props: CardProps): React.ReactNode {
  ...
  return (
    <article
      className={clsx(
        'transition-all flex flex-col items border border-gray-300 w-full max-w-[640px] aspect-[1/1] sm:aspect-[4/3] shadow-sm bg-white hover:bg-gray-100 cursor-pointer hover:shadow-xl',
    ...
    >
      ...
      <div className="transition-all relative w-full h-full hover:brightness-75 min-h-48">
        <Image className="object-cover" fill sizes="100%" src={arcicleCoverSrc} alt="article-cover" />
      </div>
      )}
    </article>
  );
